...
static void proc_ld(cpu_context* ctx)
{
    ...
    {
        u8 h_flag = (cpu_read_reg(ctx->cur_inst->reg_2) & 0xF) + 
            (ctx->fetched_data & 0xF) >= 0x10;

        u8 c_flag = (cpu_read_reg(ctx->cur_inst->reg_2) & 0xFF) + 
            (ctx->fetched_data & 0xFF) >= 0x100;

        cpu_set_flags(ctx, 0, 0, h_flag, c_flag);

        //Important to cast fetched_data to char since the value could be negative...
